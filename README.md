...
*Hệ thống IoT giám sát, cảnh báo và báo động cháy thời gian thực*

`ESP32` `MQTT` `Firebase Realtime DB` `Spring Boot` `JWT` `React + Vite`
...
1. [Kiến Trúc Tổng Quan](#1-kiến-trúc-tổng-quan-system-architecture)
2. [Kết Nối Sơ Đồ Phần Cứng](#2-kết-nối-sơ-đồ-phần-cứng-hardware-wiring--pinout)
3. [Cấu Trúc Thư Mục Dự Án](#3-cấu-trúc-thư-mục-dự-án-directory-structure)
4. [Hướng Dẫn Cài Đặt Và Vận Hành](#4-hướng-dẫn-cài-đặt-và-vận-hành-setup--installation-guide)
5. [Đặc Tả Thiết Kế Cơ Sở Dữ Liệu](#5-đặc-tả-thiết-kế-cơ-sở-dữ-liệu-database-schema)
6. [Luồng Dữ Liệu Thực Tế Hệ Thống](#6-luồng-dữ-liệu-thực-tế-hệ-thống-workflow--scenarios)
7. [Các Tính Năng Nổi Bật](#7-các-tính-năng-nổi-bật-của-hệ-thống)
...
<details open>
<summary><b>1️⃣ Thiết bị đầu cuối (ESP32)</b></summary>
...
<details open>
<summary><b>2️⃣ Cầu nối truyền thông (Python Bridge)</b></summary>
...
<details open>
<summary><b>3️⃣ Firebase Realtime Database</b></summary>
...
<details open>
<summary><b>4️⃣ Backend Server (Spring Boot)</b></summary>
...
<details open>
<summary><b>5️⃣ Frontend Web App (React + Vite)</b></summary>
...
| **I2C LCD 16x2** | VCC, GND, SDA, SCL | `GPIO 21` (SDA)<br>`GPIO 22` (SCL) | Hiển thị thông tin tại chỗ | Địa chỉ I2C mặc định `0x27` |
...
| MQTT Broker | Ví dụ: Eclipse Mosquitto (cục bộ hoặc máy chủ) |
...
> ⚠️ **Lưu ý:** Kiểm tra và sửa cấu hình Mosquitto cho phép kết nối ẩn danh (anonymous access) nếu chạy thử nghiệm nội bộ.
...
Tải file định dạng JSON chứa Service Account key của Firebase từ trang Firebase Console và lưu vào thư mục `bridge`. Sửa đường dẫn tương ứng tại biến `FIREBASE_CERT_PATH` và sửa IP MQTT Broker tương ứng trong `bridge.py`:
...
Tạo file `.env` từ file mẫu hoặc cấu hình trực tiếp các biến môi trường sau:
...
📘 Mở tài liệu API bằng Swagger UI tại: `http://localhost:8080/swagger-ui/index.html` (hoặc `/swagger-ui.html`).
...
Tạo file `.env` chứa URL gọi API đến Backend:
...
🌐 Mở trình duyệt truy cập ứng dụng tại: `http://localhost:5173`
...
| 🛡️ **Bảo mật dữ liệu tối đa** | Nhờ cơ chế API Proxy, client (Frontend/Mobile) không tương tác trực tiếp với Firebase hoặc Database chứa tài khoản, ngăn ngừa hoàn toàn nguy cơ lộ các API Key hoặc cấu hình máy chủ cơ sở dữ liệu. |
